...
describe("anaerobicReserve (bounded above-LT2 capacity for short predictions only)", () => {
  // Default ceilingParams (f0=0.94, fInf=0.38, tauMin=250, lt2Fraction=0.85)
  // cross back below the cap at ~44 minutes -- see anaerobicReserve.test.ts's
  // own reserveCrossoverMin coverage for the exact value.
  const reserve = { reserveKJPerKg: 0.3 };
  const shortCourse = baseInputs({ segments: makeSegments(100, 50, 0) }); // ~5km flat, well under the crossover
  const marathonCourse = baseInputs({ segments: makeSegments(844, 50, 0) }); // 42.2km flat, well over it

  it("findSustainableTheta: long race is byte-identical with vs without a reserve configured", () => {
    const without = findSustainableTheta(marathonCourse);
    const withReserve = findSustainableTheta({ ...marathonCourse, anaerobicReserve: reserve });
    expect(withReserve).toEqual(without);
  });

  it("findThetaForTargetTime: long target is byte-identical with vs without a reserve configured", () => {
    const seed = findSustainableTheta(marathonCourse);
    const target = seed.result.finishTimeS * 1.1;
    const without = findThetaForTargetTime(marathonCourse, target);
    const withReserve = findThetaForTargetTime({ ...marathonCourse, anaerobicReserve: reserve }, target);
    expect(withReserve).toEqual(without);
  });

  it("findFlatPacedFinishTime: long race is byte-identical with vs without a reserve configured", () => {
    const without = findFlatPacedFinishTime(marathonCourse);
    const withReserve = findFlatPacedFinishTime({ ...marathonCourse, anaerobicReserve: reserve });
    expect(withReserve).toEqual(without);
  });

  it("findSustainableTheta: short race finishes measurably faster with a reserve than without", () => {
    const without = findSustainableTheta(shortCourse);
    const withReserve = findSustainableTheta({ ...shortCourse, anaerobicReserve: reserve });
    expect(without.result.feasible).toBe(true);
    expect(withReserve.result.feasible).toBe(true);
    expect(withReserve.result.finishTimeS).toBeLessThan(without.result.finishTimeS);
  });

  it("findFlatPacedFinishTime: short race finishes measurably faster with a reserve than without", () => {
    const without = findFlatPacedFinishTime(shortCourse);
    const withReserve = findFlatPacedFinishTime({ ...shortCourse, anaerobicReserve: reserve });
    expect(withReserve.result.finishTimeS).toBeLessThan(without.result.finishTimeS);
  });

  it("a course whose own baseline duration lands near the crossover still produces a sane, self-consistent result (no split-brain scan/bisection behavior)", () => {
    // ~9.5km -- baseline duration should land close to the ~44min crossover
    // for these default params, without needing to hit it exactly.
    const borderline = baseInputs({ segments: makeSegments(190, 50, 0) });
    const result = findFlatPacedFinishTime({ ...borderline, anaerobicReserve: reserve });
  ...
  it("has no effect at all when omitted, on every entry point (existing behavior preserved exactly)", () => {
    // Redundant with running the full pre-existing suite unmodified, but
    // pins down explicitly: the new field is optional/undefined by default,
    // so every call site not using it is provably unaffected by this
    // feature existing at all.
    expect(findSustainableTheta(shortCourse).result.finishTimeS).toBe(
      findSustainableTheta({ ...shortCourse, anaerobicReserve: undefined }).result.finishTimeS,
    );
  });
});
